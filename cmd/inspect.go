...
var inspectCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		c, cerr := recmd.SelectCmd(homeDir, "commandHash", value)

		if cerr != nil {
			fmt.Fprintf(os.Stderr, "Unable to read history file: %s\n", err)
		}

		// Unable to find the command by hash. Try the comand string instnead.
		if c.CmdHash == "" {
			c, cerr = recmd.SelectCmd(homeDir, "commandString", value)
		...
		if c.CmdHash == "" {
			fmt.Fprintf(os.Stdout, "Unaable to find command\n")
			return
		}
