...
func TestMain(m *testing.M) {
	...
	CreateCmdHistoryFile(testdataDir)
...
func TestSearchCommand(t *testing.T) {
	...
	cmd := NewCommand("docker system prune -f", "Prune docker containers")
	cmd2 := NewCommand("docker rmi $(docker images -f dangling=true -q)", "Remove all images with no tag")
	cmd3 := NewCommand("sleep 3; xset dpms force off", "Darken screen with a delay")
	...
	cmds, _ := SearchCmd(testdataDir, "docker")

	if len(cmds) != 1 {
		t.Error("Unable to find docker command")
	}

	cmds, _ = SearchCmd(testdataDir, "remove")
